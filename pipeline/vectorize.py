...
COLOR_KEYS = {"strokes", "line_mode", "line_min_px", "paper_thresh", "line_dark", "line_s_max", "min_px", "merge_de", "min_frac", "white_layer", "smooth", "median", "open_px", "light_thresh", "light_min_px"}
...
def quantize(bgr, k=8, exclude=None, merge_de=11.0, min_frac=0.03, seed=1, assign=None):
        ...
        for a in roots:
            for b in roots:
                ...
                if np.linalg.norm(centers[a] - centers[b]) < merge_de:
                    ...
                    counts[a] = w; parent[b] = a; changed = True
    ...
    while changed:
        ...
        for a in roots:
            ...
            counts[b] += counts[a]; parent[a] = b; changed = True; break
    roots = sorted(set(find(i) for i in range(k)))
...
def _reassign_ring(labels, ring):
    """Give every pixel in `ring` the label of its nearest non-ring pixel."""
    dist, idx = cv2.distanceTransformWithLabels((ring).astype(np.uint8), cv2.DIST_L2, 3, labelType=cv2.DIST_LABEL_PIXEL)
    flat = labels.ravel()
    ys, xs = np.where(~ring)
    lut = np.zeros(idx.max() + 1, np.int32)
    lut[idx[~ring]] = flat[ys * labels.shape[1] + xs]
    out = labels.copy(); out[ring] = lut[idx[ring]]
    return out
...
def trace_color(bgr, k=8, scale=3, paper_thresh=232, line_dark=125, line_s_max=70, min_px=60, merge_de=11.0,
                ...
                line_mode="auto", line_min_px=20, **opts):
    ...
    labels, centers = quantize(up, k, exclude=lines | ring, merge_de=merge_de, min_frac=min_frac, assign=sharp)
    ...
    groups = _strokes.trace_strokes(lines) if strokes else []
    if groups:
        ...
        zone = cv2.dilate(lines.astype(np.uint8), k_).astype(bool)
        for l in fills:
            grown = cv2.dilate(l["mask"].astype(np.uint8), k_).astype(bool)
            l["mask"] = l["mask"] | (grown & zone)
    for l in fills:
        l["paths"] = _trace_mask(l.pop("mask"), scale, prescaled=True, **{**SMOOTH_TRACE, **opts})
    if groups:
        line_layer = {"hex": line_hex, "area": int(lines.sum()), "lum": 0.0, "stroke": True, "groups": groups,
                      "paths": [d for g in groups for d in g["paths"]]}
    else:
        line_layer = {"hex": line_hex, "area": int(lines.sum()), "lum": 0.0,
                      "paths": _trace_mask(lines, scale, prescaled=True, **{**SMOOTH_TRACE, **opts})}
    return fills + [line_layer]
