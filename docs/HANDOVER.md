...
> that dresses a real typeset page. **The next job is the font lineage (PLAN §1, §8 phase 4)
> and the licensing decision (PLAN §6), which is Abdullah's.** Nothing is published: the npm
> package is deliberately `private` and the release workflow refuses to publish it.
...
* 8 mushafs × 3 types = 24 assets, all generated by `python -m qa build` with no manual boxes,
